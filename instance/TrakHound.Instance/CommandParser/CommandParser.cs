    ...
    internal static class CommandParser
    {
        ...
        public static async Task<bool> Run(Type type, string[] args)
        {
            ...
            if (match != null)
            {
                ...
                var constructorParameters = !commandParameters.IsNullOrEmpty() ? commandParameters.Values.ToArray() : new object[] { };

                // Run Method
                if (match.Method.ReturnType == typeof(Task))
                {
                    await (Task)match.Method.Invoke(null, constructorParameters);
                    //await (Task)match.Method.Invoke(null, commandParameters?.Values.ToArray());
                }
                else
                {
                    match.Method.Invoke(null, constructorParameters);
