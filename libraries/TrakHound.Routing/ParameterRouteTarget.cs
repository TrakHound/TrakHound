...
{
    class ParameterRouteTarget : IRouteTarget
    {
        ...
        private static async Task<RouteTargetResponse<TReturn>> RunDriverTarget<TDriver, TEntity, TReturn>(
            string routerId,
            IParameterRouteRequest request,
            ITrakHoundLogger logger,
            IRouteDriverTarget target,
            TDriver driver,
            Func<ParameterRouteTargetDriverRequest<TDriver, TEntity>, Task<TrakHoundResponse<TReturn>>> driverFunction,
            Func<ParameterRouteTargetRouterRequest<TEntity>, Task<TrakHoundResponse<TReturn>>> routerFunction,
            Func<IParameterRouteRequest, IEnumerable<TrakHoundResult<TReturn>>, IParameterRouteRequest> processFunction
            )
            ...
            if (target != null && driver != null)
            {
                if (driver.IsAvailable)
                {
                    try
                    {
                        ...
                        var driverResponse = await driverFunction(new ParameterRouteTargetDriverRequest<TDriver, TEntity>(target, driver, request));
                        if (driverResponse.HasResults) responseResults.AddRange(driverResponse.Results);

                        if (_verboseLogging) logger.LogTrace($"{request.Id} : Router ({routerId}) : {request.Name} => ({driver.Id}) : Driver Target Response in {TimeSpan.FromTicks(driverResponse.Duration).TotalMilliseconds}ms");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            foreach (var requestQuery in request.Queries) responseResults.Add(new TrakHoundResult<TReturn>(driver.Id, requestQuery.Query, TrakHoundResultType.InternalError));
                        }
                        else
                        {
                ...
                {
                    responseResults.Add(new TrakHoundResult<TReturn>(driver.Id, request.Id, TrakHoundResultType.NotAvailable));
                }
            }
            ...
            if (target != null)
            {
                ...
                else
                {
                    if (!request.Queries.IsNullOrEmpty())
                    {
                        ...
                        {
                            responseResults.Add(new TrakHoundResult<TReturn>(routerId, query.Query, TrakHoundResultType.RouteNotConfigured));
                        }
                    }
            ...
            where TDriver : ITrakHoundDriver
        {
            ...
            if (target != null)
            {
                ...
                {
                    // Read the Response from the Driver
                    var routerResponse = await routerFunction(new ParameterRouteTargetRouterRequest<TEntity>(target, router, request));
                    if (routerResponse.HasResults) responseResults.AddRange(routerResponse.Results);
                    //if (!routerResponse.Results.IsNullOrEmpty()) responseResults.AddRange(routerResponse.Results);

                    if (_verboseLogging) logger.LogTrace($"{request.Id} : Router ({routerId}) : {request.Name} => ({router.Id}) : Driver Target Response in {TimeSpan.FromTicks(routerResponse.Duration).TotalMilliseconds}ms");
            ...
            where TDriver : ITrakHoundDriver
        {
            ...
            var redirectRequestInstance = new ParameterRouteRequest();
            redirectRequestInstance.Name = request.Name;
            redirectRequestInstance.Id = request.Id;
            redirectRequestInstance.Parameters = request.Parameters;
            redirectRequestInstance.AddQueries(request.Queries);
            IParameterRouteRequest redirectRequest = redirectRequestInstance;
            ...
            if (target != null)
            {
                ...
                foreach (var resultType in resultTypes)
                {
                    ...
                    if (_verboseLogging)
                    {
                        ...
                        typeRequestsCount = typeRequests.Count();
                    ...
                    if (!target.Redirects.IsNullOrEmpty())
                    {
                        ...
                        if (redirect != null)
                        {
                            if (_verboseLogging) logger.LogTrace($"{request.Id} : > Redirect : Router ({routerId}) : {request.Name} => ({sourceId}) Driver Target : Redirecting {typeRequestsCount} Requests on \"{resultType}\"");
                            ...
                            if (redirectResponse.IsSuccess && !redirect.Options.IsNullOrEmpty())
                            {
                                if (redirect.Options.Any(o => o == RouteRedirectOptions.Publish))
                                {
                                    ...
                                    {
                                        if (_verboseLogging) logger.LogTrace($"{redirectRequest.Id} : Option [{RouteRedirectOptions.Publish}] => Router ({routerId}) set to {RouteRedirectOptions.Publish} {redirectResponse.SuccessResults.Count()} Results");
                            ...
                            if (redirectResponse.IsEmpty && !redirect.Options.IsNullOrEmpty())
                            {
                                if (redirect.Options.Any(o => o == RouteRedirectOptions.Empty))
                                {
                                    ...
                                    if (target.Id != sourceId)
                                    {
                                        if (_verboseLogging) logger.LogTrace($"{redirectRequest.Id} : Option [{RouteRedirectOptions.Empty}] => Router ({routerId}) set to {RouteRedirectOptions.Empty} {redirectResponse.EmptyResults.Count()} Results");
        ...
        private static async Task<RouteTargetResponse<TReturn>> RunTarget<TDriver, TEntity, TReturn>(
            ...
            Func<ParameterRouteTargetDriverRequest<TDriver, TEntity>, Task<TrakHoundResponse<TReturn>>> driverFunction,
            Func<ParameterRouteTargetRouterRequest<TEntity>, Task<TrakHoundResponse<TReturn>>> routerFunction,
            Func<IParameterRouteRequest, IEnumerable<TrakHoundResult<TReturn>>, IParameterRouteRequest> processFunction = null,
            ...
            where TDriver : ITrakHoundDriver
        {
            if (target != null)
            {
                ...
                {
                    if (_verboseLogging) logger.LogTrace($"{request.Id} : > Redirect : Router ({routerId}) : Router Target ({sourceTarget.Id}) Redirected to ({target.Id})");
                }
            ...
            where TDriver : ITrakHoundDriver
        {
            ...
            if (sourceTarget == null)
            {
                if (_verboseLogging) logger.LogTrace($"{request.Id} : {request.Name} Request to Router ({routerId}) Received");
            }
            ...
            if (sourceTarget == null)
            {
                if (_verboseLogging) logger.LogTrace($"{request.Id} : {request.Name} Request to Router ({routerId}) Completed in {stpw.ElapsedMilliseconds}ms");
            }
        ...
        public static async Task<ITrakHoundConsumer<TReturn>> Subscribe<TDriver, TEntity, TReturn>(
            ...
            ITrakHoundLogger logger,
            IEnumerable<IRouteTarget> targets,
            Func<ParameterRouteTargetDriverRequest<TDriver, TEntity>, Task<ITrakHoundConsumer<TReturn>>> driverFunction,
            Func<ParameterRouteTargetRouterRequest<TEntity>, Task<ITrakHoundConsumer<TReturn>>> routerFunction,
            IRouteTarget sourceTarget = null
            )
            where TDriver : ITrakHoundDriver
        {
            ...
            if (!targets.IsNullOrEmpty())
            {
                foreach (var target in targets)
                {
                    ...
                    if (target.GetType() == typeof(ParameterRouteDriverTarget))
                    {
                        ...
                        if (!drivers.IsNullOrEmpty())
                        {
                            foreach (var driver in drivers)
                            {
                                ...
                                consumer = await driverFunction(new ParameterRouteTargetDriverRequest<TDriver, TEntity>(target, driver, request));
                    ...
                    else if (target.GetType() == typeof(ParameterRouteRouterTarget))
                    {
                        ...
                        consumer = await routerFunction(new ParameterRouteTargetRouterRequest<TEntity>(target, routerTarget.Router, request));
                        if (consumer != null) consumers.Add(consumer);
                    }
                }
            }
