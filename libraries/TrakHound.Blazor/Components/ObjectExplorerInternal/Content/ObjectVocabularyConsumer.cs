...
{
    public class ObjectVocabularyConsumer : IContentConsumer
    {
        private readonly ITrakHoundClient _client;
        private readonly string _consumerId;
        private ITrakHoundConsumer<IEnumerable<ITrakHoundObjectVocabularyEntity>> _consumer;


        public event ContentHandler ValueUpdated;


        public ObjectVocabularyConsumer(ITrakHoundClient client, string consumerId)
        {
            _client = client;
        ...
        public async Task Load(IEnumerable<ITrakHoundObjectEntity> objects)
        {
            ...
            {
                var contentObjs = objects.Where(o => o.ContentType == TrakHoundObjectContentTypes.Vocabulary);
                if (!contentObjs.IsNullOrEmpty())
                {
                    var objectUuids = contentObjs.Select(o => o.Uuid).Distinct();

                    var entities = await _client.System.Entities.Objects.Vocabulary.QueryByObjectUuid(objectUuids);
        ...
        private async Task Subscribe(IEnumerable<string> objectUuids)
        {
            if (_client != null)
            {
                _consumer = await _client.System.Entities.Objects.Vocabulary.SubscribeByObjectUuid(objectUuids, consumerId: _consumerId);
                if (_consumer != null)
                {
        ...
        private async void OnReceived(object sender, IEnumerable<ITrakHoundObjectVocabularyEntity> entities)
        {
            foreach (var entity in entities)
            {
        ...
        private async Task UpdateValue(ITrakHoundObjectVocabularyEntity entity)
        {
