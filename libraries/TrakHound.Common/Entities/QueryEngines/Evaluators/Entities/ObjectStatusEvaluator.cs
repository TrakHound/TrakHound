...
{
    public class ObjectStatusEvaluator : EntityEvaluator, IEvaluator
    {
        public ObjectStatusEvaluator(ConditionObjectQuery query, ITrakHoundObjectEntityModel conditionObject)
        {
        ...
        public static async Task<IEnumerable<IEvaluatorResult>> Evaluate(QueryStatement statement, ITrakHoundSystemEntitiesClient client, IEnumerable<ObjectStatusEvaluator> evaluators)
        {
            ...
            {
                IEnumerable<ITrakHoundObjectStateEntityModel> entities;
                ...
                if (statement.Start > 0 || statement.Stop > 0)
                {
                    ...
                    entities = await client.Objects.State.QueryModelsByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid), startUnix, stopUnix);
                }
                else
                {
                    entities = await client.Objects.State.LatestModelsByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid));
                }
                    ...
                    foreach (var entity in entities)
                    {
                        ...
                        if (!entityEvaluators.IsNullOrEmpty())
                        {
                            foreach (var entityEvaluator in entityEvaluators)
                            {
                                ...
                                if (ProcessValue(entityEvaluator.Condition.Operator, entity.Type, value))
                                {
