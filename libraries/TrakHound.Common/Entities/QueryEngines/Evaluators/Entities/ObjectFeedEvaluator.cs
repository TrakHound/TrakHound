...
{
    public class ObjectFeedEvaluator : EntityEvaluator, IEvaluator
    {
        public ObjectFeedEvaluator(ConditionObjectQuery query, ITrakHoundObjectEntityModel conditionObject)
        {
        ...
        public static async Task<IEnumerable<IEvaluatorResult>> Evaluate(QueryStatement statement, ITrakHoundSystemEntitiesClient client, IEnumerable<ObjectFeedEvaluator> evaluators)
        {
            ...
            {
                IEnumerable<ITrakHoundObjectFeedEntity> entities;
                ...
                if (statement.Start > 0 || statement.Stop > 0)
                {
                    ...
                    entities = await client.Objects.Feed.QueryByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid), startUnix, stopUnix);
                }
                else
                {
                    entities = await client.Objects.Feed.LatestByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid));
                }
                    ...
                    foreach (var entity in entities)
                    {
                        ...
                        if (!entityEvaluators.IsNullOrEmpty())
                        {
                            foreach (var entityEvaluator in entityEvaluators)
                            {
                                ...
                                if (ProcessValue(entityEvaluator.Condition.Operator, QueryStatement.ProcessFunctions(entity.Message), QueryStatement.ProcessFunctions(value)))
                                {
