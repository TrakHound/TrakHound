...
{
    public class ObjectTimestampEvaluator : EntityEvaluator, IEvaluator
    {
        public ObjectTimestampEvaluator(ConditionObjectQuery query, ITrakHoundObjectEntityModel conditionObject)
        {
        ...
        public static async Task<IEnumerable<IEvaluatorResult>> Evaluate(QueryStatement statement, ITrakHoundSystemEntitiesClient client, IEnumerable<ObjectTimestampEvaluator> evaluators)
        {
            ...
            {
                var entities = await client.Objects.Timestamp.QueryByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid));
        ...
        private static bool ProcessValue(TrakHoundConditionOperator operatorType, long target, string pattern)
        {
            var value = pattern.ToDateTime().ToUnixTime();
