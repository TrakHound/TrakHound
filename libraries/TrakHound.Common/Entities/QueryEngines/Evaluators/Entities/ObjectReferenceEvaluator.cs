...
{
    public class ObjectReferenceEvaluator : EntityEvaluator, IEvaluator
    {
        public ObjectReferenceEvaluator(ConditionObjectQuery query, ITrakHoundObjectEntityModel conditionObject)
        {
        ...
        public static async Task<IEnumerable<IEvaluatorResult>> Evaluate(QueryStatement statement, ITrakHoundSystemEntitiesClient client, IEnumerable<ObjectReferenceEvaluator> evaluators)
        {
            ...
            {
                var entities = await client.Objects.Reference.QueryModelsByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid));
                    ...
                    foreach (var entity in dEntities)
                    {
                        ...
                        if (!entityEvaluators.IsNullOrEmpty())
                        {
                            foreach (var entityEvaluator in entityEvaluators)
                            {
                                ...
                                if (ProcessValue(entityEvaluator.Condition.Operator, entity.Target, QueryStatement.ProcessFunctions(value)))
                                {
                                    foreach (var select in statement.Select)
                                    {
                                        ...
                                        if (selectQuery != null)
                                        {
                                            ...
                                            if (selectTarget == null) selectTarget = TrakHoundObjectExpression.Match(selectQuery.Value, entity.Target)?.FirstOrDefault();
