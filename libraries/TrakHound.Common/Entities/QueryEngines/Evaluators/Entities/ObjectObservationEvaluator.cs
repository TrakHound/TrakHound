...
{
    public class ObjectObservationEvaluator : EntityEvaluator, IEvaluator
    {
        public ObjectObservationEvaluator(ConditionObjectQuery query, ITrakHoundObjectEntityModel conditionObject)
        {
        ...
        public static async Task<IEnumerable<IEvaluatorResult>> Evaluate(QueryStatement statement, ITrakHoundSystemEntitiesClient client, IEnumerable<ObjectObservationEvaluator> evaluators)
        {
            ...
            {
                IEnumerable<ITrakHoundObjectObservationEntity> entities = null;
                //IEnumerable<ITrakHoundObjectObservationEntity> entities;
                ...
                if (statement.Start > 0 || statement.Stop > 0)
                {
                    ...
                    var skip = statement.Skip ?? 0;
                    var take = statement.Take ?? QueryStatement.DefaultTake;
                    ...
                    {
                        rangeQueries.Add(new TrakHoundRangeQuery(objectUuid, startUnix, stopUnix));
                    }


                    //entities = await client.Objects.Observation.QueryByRange(rangeQueries, conditionGroups, skip, take);
                ...
                {
                    entities = await client.Objects.Observation.LatestByObjectUuid(evaluators.Select(o => o.ConditionObject.Uuid));
                }
                        ...
                        if (!entityEvaluators.IsNullOrEmpty())
                        {
                            foreach (var entityEvaluator in entityEvaluators)
                            {
                                ...
                                if (ProcessValue(entityEvaluator.Condition.Operator, QueryStatement.ProcessFunctions(entity.Value), QueryStatement.ProcessFunctions(value)))
                                {
                                    ...
                                    //foreach (var select in GroupBy)
                                    //{
                                    //    var selectQuery = statement.Scope.GetVariable(select);
                                    //    if (selectQuery != null)
                                    //    {
                                    //        var result = new EntityEvaluatorResult();
                                    //        result.Id = Guid.NewGuid().ToString();
        ...
        private static TrakHoundConditionGroupQuery CreateConditionGroupQuery(QueryStatement statement, QueryConditionGroup inputConditionGroup)
        {
            ...
            var conditions = new List<TrakHound.TrakHoundConditionQuery>();
                ...
                foreach (var inputCondition in inputConditionGroup.Conditions)
                {
                    ...
                    if (nameVariable != null)
                    {
                        ...
                        else
                        {
                            ...
                            conditions.Add(new TrakHound.TrakHoundConditionQuery(target, inputCondition.Operator, valueVariable.Value));
