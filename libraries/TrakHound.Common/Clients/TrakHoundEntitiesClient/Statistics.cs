...
namespace TrakHound.Clients
{
    public abstract partial class TrakHoundEntitiesClientBase
    {
        ...
        public async Task<IEnumerable<TrakHoundStatistic>> GetLatestStatistics(string objectPath, DateTime start, DateTime stop, TimeSpan span, int skip = _defaultSkip, int take = _defaultTake, SortOrder sortOrder = _defaultSortOrder, string routerId = null)
        {
            if (!string.IsNullOrEmpty(objectPath) && start > DateTime.MinValue && stop > DateTime.MinValue)
            {
                var url = Url.Combine(ApiRoute, _objectsRoute, $"{_statisticsRoute}/latest");
        ...
        public async Task<IEnumerable<TrakHoundStatistic>> GetLatestStatistics(string objectPath, string startExpression, string stopExpression, string spanExpression, int skip = _defaultSkip, int take = _defaultTake, SortOrder sortOrder = _defaultSortOrder, string routerId = null)
        {
