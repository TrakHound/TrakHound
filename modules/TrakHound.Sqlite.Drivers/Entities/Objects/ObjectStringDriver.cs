...
{
    public class ObjectStringDriver : SqliteEntityDriver<ITrakHoundObjectStringEntity, DatabaseObjectString>, IObjectStringQueryDriver
    {
        public ObjectStringDriver() { }

        public ObjectStringDriver(ITrakHoundDriverConfiguration configuration) : base(configuration) 
        {
            EntityName = "trakhound_objects_string";
        ...
        protected async override Task<bool> OnPublish(IEnumerable<ITrakHoundObjectStringEntity> entities)
        {
        ...
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectString>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectString>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
