...
{
    public class ObjectVocabularySetDriver : SqliteEntityDriver<ITrakHoundObjectVocabularySetEntity, DatabaseObjectVocabularySet>, IObjectVocabularySetQueryDriver
    {
        ...
        {
            EntityName = "trakhound_objects_vocabulary_set";
        ...
        protected async override Task<bool> OnPublish(IEnumerable<ITrakHoundObjectVocabularySetEntity> entities)
        {
        ...
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectVocabularySet>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectVocabularySet>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
