...
{
    public class ObjectBooleanDriver : SqliteEntityDriver<ITrakHoundObjectBooleanEntity, DatabaseObjectBoolean>, IObjectBooleanQueryDriver
    {
        public ObjectBooleanDriver() { }

        public ObjectBooleanDriver(ITrakHoundDriverConfiguration configuration) : base(configuration) 
        {
            EntityName = "trakhound_objects_boolean";
        ...
        protected async override Task<bool> OnPublish(IEnumerable<ITrakHoundObjectBooleanEntity> entities)
        {
        ...
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectBoolean>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectBoolean>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
