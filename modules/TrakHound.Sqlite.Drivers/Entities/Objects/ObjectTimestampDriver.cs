...
{
    public class ObjectTimestampDriver : SqliteEntityDriver<ITrakHoundObjectTimestampEntity, DatabaseObjectTimestamp>, IObjectTimestampQueryDriver
    {
        public ObjectTimestampDriver() { }

        public ObjectTimestampDriver(ITrakHoundDriverConfiguration configuration) : base(configuration) 
        {
            EntityName = "trakhound_objects_timestamp";
        ...
        protected async override Task<bool> OnPublish(IEnumerable<ITrakHoundObjectTimestampEntity> entities)
        {
        ...
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectTimestamp>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectTimestamp>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
        ...
        public async Task<TrakHoundResponse<ITrakHoundObjectTimestampEntity>> Query(IEnumerable<string> objectUuids, long min, long max)
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectTimestamp>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectTimestamp>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
