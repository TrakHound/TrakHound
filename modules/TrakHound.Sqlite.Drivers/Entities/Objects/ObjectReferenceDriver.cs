...
{
    public class ObjectReferenceDriver : SqliteEntityDriver<ITrakHoundObjectReferenceEntity, DatabaseObjectReference>, IObjectReferenceQueryDriver
    {
        public ObjectReferenceDriver() { }

        public ObjectReferenceDriver(ITrakHoundDriverConfiguration configuration) : base(configuration) 
        {
            EntityName = "trakhound_objects_reference";
        ...
        protected async override Task<bool> OnPublish(IEnumerable<ITrakHoundObjectReferenceEntity> entities)
        {
        ...
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectReference>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectReference>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
