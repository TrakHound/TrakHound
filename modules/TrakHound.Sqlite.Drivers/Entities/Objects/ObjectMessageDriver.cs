...
{
    public class ObjectMessageDriver : SqliteEntityDriver<ITrakHoundObjectMessageEntity, DatabaseObjectMessage>, IObjectMessageQueryDriver
    {
        public ObjectMessageDriver() { }

        public ObjectMessageDriver(ITrakHoundDriverConfiguration configuration) : base(configuration) 
        {
            EntityName = "trakhound_objects_message";
        ...
        protected async override Task<bool> OnPublish(IEnumerable<ITrakHoundObjectMessageEntity> entities)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                item.Topic = entity.Topic;
                item.ContentType = entity.ContentType;
                item.Retain = entity.Retain;
                item.Qos = entity.Qos;
        ...
        {
            Func<IEnumerable<string>, Task<IEnumerable<DatabaseObjectMessage>>> readFunction = async (ids) =>
            {
                var dbEntities = ExecuteConditionQuery<DatabaseObjectMessage>($"select {TableColumns} from {TableName} where", "object_uuid", objectUuids);
