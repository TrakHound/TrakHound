...
namespace TrakHound.Sqlite.Drivers
{
        ...
        where TDatabaseEntity : IDatabaseEntity<TEntity>
    {
        ...
        public static async Task<TrakHoundResponse<TrakHoundAggregate>> ProcessResponse(SqliteDriver driver, string requestedId, Func<IEnumerable<string>, Task<IEnumerable<DatabaseAggregate>>> readFunction)
        {
            var requestedIds = new List<string>();
            if (requestedId != null) requestedIds.Add(requestedId);
        ...
        public static async Task<TrakHoundResponse<TrakHoundAggregate>> ProcessResponse(SqliteDriver driver, IEnumerable<string> requestedIds, Func<IEnumerable<string>, Task<IEnumerable<DatabaseAggregate>>> readFunction)
        {
            var stpw = System.Diagnostics.Stopwatch.StartNew();
            var results = new List<TrakHoundResult<TrakHoundAggregate>>();
            ...
            if (driver != null)
            {
                if (!requestedIds.IsNullOrEmpty())
                {
                        ...
                        if (!responses.IsNullOrEmpty())
                        {
                            foreach (var requestedId in requestedIds)
                            {
                                ...
                                if (!idResponses.IsNullOrEmpty())
                                {
                                    foreach (var response in idResponses)
                                    {
                                        results.Add(new TrakHoundResult<TrakHoundAggregate>(driver.Id, requestedId, TrakHoundResultType.Ok, new TrakHoundAggregate(response.RequestedId, response.Value)));
                                    }
                                }
                                else
                                {
                                    results.Add(new TrakHoundResult<TrakHoundAggregate>(driver.Id, requestedId, TrakHoundResultType.NotFound));
                                }
                            }
                        }
                    }
                    else
                    {
                        foreach (var requestedId in requestedIds)
                        {
                            results.Add(new TrakHoundResult<TrakHoundAggregate>(driver.Id, requestedId, TrakHoundResultType.NotAvailable));
                        }
                    }
                }
                else
                {
                    results.Add(TrakHoundResult<TrakHoundAggregate>.BadRequest(driver.Id));
                }
            }
            else
            {
                results.Add(TrakHoundResult<TrakHoundAggregate>.InternalError(driver.Id));
            }

            stpw.Stop();
            return new TrakHoundResponse<TrakHoundAggregate>(results, stpw.ElapsedTicks);
        }


        public static async Task<TrakHoundResponse<TrakHoundAggregateWindow>> ProcessResponse(SqliteDriver driver, string requestedId, Func<IEnumerable<string>, Task<IEnumerable<DatabaseAggregateWindow>>> readFunction)
        {
            var requestedIds = new List<string>();
            if (requestedId != null) requestedIds.Add(requestedId);
        ...
        public static async Task<TrakHoundResponse<TrakHoundAggregateWindow>> ProcessResponse(SqliteDriver driver, IEnumerable<string> requestedIds, Func<IEnumerable<string>, Task<IEnumerable<DatabaseAggregateWindow>>> readFunction)
        {
            var stpw = System.Diagnostics.Stopwatch.StartNew();
            var results = new List<TrakHoundResult<TrakHoundAggregateWindow>>();
            ...
            if (driver != null)
            {
                if (!requestedIds.IsNullOrEmpty())
                {
                        ...
                        if (!responses.IsNullOrEmpty())
                        {
                            foreach (var requestedId in requestedIds)
                            {
                                ...
                                if (!idResponses.IsNullOrEmpty())
                                {
                                    foreach (var response in idResponses)
                                    {
                                        results.Add(new TrakHoundResult<TrakHoundAggregateWindow>(driver.Id, requestedId, TrakHoundResultType.Ok, new TrakHoundAggregateWindow(response.RequestedId, response.Start, response.End, response.Value)));
                                    }
                                }
                                else
                                {
                                    results.Add(new TrakHoundResult<TrakHoundAggregateWindow>(driver.Id, requestedId, TrakHoundResultType.NotFound));
                                }
                            }
                        }
                    }
                    else
                    {
                        foreach (var requestedId in requestedIds)
                        {
                            results.Add(new TrakHoundResult<TrakHoundAggregateWindow>(driver.Id, requestedId, TrakHoundResultType.NotAvailable));
                        }
                    }
                }
                else
                {
                    results.Add(TrakHoundResult<TrakHoundAggregateWindow>.BadRequest(driver.Id));
                }
            }
            else
            {
                results.Add(TrakHoundResult<TrakHoundAggregateWindow>.InternalError(driver.Id));
            }

            stpw.Stop();
            return new TrakHoundResponse<TrakHoundAggregateWindow>(results, stpw.ElapsedTicks);
        }
